...
const getHistoryBySede = async (req, res) => {
    ...
    let consultations = await Consultation.find().populate([{ path: "patient", populate: { path: "personData" } }, { path: "doctor", populate: { path: "personData" } }, { path: "campus", populate: { path: "user", match: { _id: userId } } }]).sort('date hour');
    consultations.forEach(consultation => {
    ...
    let treatmentAppointments = await TreatmentAppointment.find().populate([{ path: "doctor", populate: { path: "personData" } }, { path: "campus", populate: { path: "user", match: { _id: userId } } }, { path: "treatmentDetail", populate: { path: "patient", populate: { path: "personData" } } } ]).sort('date hour');
    treatmentAppointments.forEach(treatmentAppointment => {
