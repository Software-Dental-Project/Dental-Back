...
const createFromCampus = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const populatedCampusDoctor = await CampusesDoctors.findById(campusDoctorStored._id).populate([{path: "doctor", populate: [{ path: "personData", select: '-__v' }, { path: 'speciality', select: 'name -_id' }], select: 'personData speciality' }, {path: 'campus', populate: { path: 'clinic', select: 'user -_id'}, select: 'user clinic -_id'}]).select('doctor -_id');
...
const getByMyCampus = async (req, res) => {
    ...
    CampusesDoctors.find().populate([{ path: "doctor", populate: [{ path: "personData", select: '-__v' }, { path: "speciality", select: 'name -_id'}], select: 'personData speciality tuitionNumber' }, { path: "campus", match: { user: userId }, populate: { path: 'clinic', select: 'user -_id' }, select: 'user clinic -_id' }]).select('doctor -_id').then(campusesDoctors => {
