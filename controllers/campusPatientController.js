...
const createFromCampus = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const populatedCampusPatient = await CampusesPatients.findById(campusPatientStored._id).populate([{path: "patient", populate: { path: "personData", select: '-__v' }, select: 'personData' }, {path: 'campus', populate: { path: 'clinic', select: 'user -_id' },select: 'user clinic -_id'}]).select('patient -_id');
...
const getByMyCampus = async (req, res) => {
    ...
    CampusesPatients.find().populate([{ path: "patient", populate: { path: "personData", select: '-__v' }, select: 'personData' }, { path: "campus", match: { user: userId }, populate: { path: 'clinic', select: 'user -_id' }, select: 'user clinic -_id' }]).select('patient campus -_id').then(campusesPatients => {
        campusesPatients = campusesPatients.filter(campusPatient => campusPatient.campus);
